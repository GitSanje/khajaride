...
```bash
git clone https://github.com/gitSanje/khajaride.git
cd go-khajaride
```
...
The backend uses environment variables prefixed with `KHAJARIDE_`. Key variables include:

- `KHAJARIDE_DATABASE_*` - PostgreSQL connection settings
- `KHAJARIDE_SERVER_*` - Server configuration
- `KHAJARIDE_AUTH_*` - Authentication settings
- `KHAJARIDE_REDIS_*` - Redis connection
- `KHAJARIDE_EMAIL_*` - Email service configuration
- `KHAJARIDE_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

This khajaride follows clean architecture principles:
